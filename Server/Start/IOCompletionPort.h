...
private:
	...
	void WokerThread()
	{
		...
		while (mIsWorkerRun)
		{
			...
			if (eIOOperation::RECV == pOverlappedEx->m_eOperation)
			{
				...
				// PATCH: 클라이언트에서 보내는 패킷을 처리하기 위한 임시 방편.
				char msgWithoutHeader[1024] = "";
				memcpy(msgWithoutHeader, pClientInfo->m_recvBuf + 5, dwIoSize - 5);

				//printf("[수신] bytes : %d , msg : %s\n", dwIoSize, pClientInfo->m_szBuf);
				printf("[수신] bytes : %d , msg : %s(Without Header)\n", dwIoSize, msgWithoutHeader);
			...
			else if (eIOOperation::SEND == pOverlappedEx->m_eOperation)
			{
				// PATCH: 클라이언트에서 보내는 패킷을 처리하기 위한 임시 방편.
				char msgWithoutHeader[1024] = "";
				memcpy(msgWithoutHeader, pClientInfo->m_sendBuf + 5, dwIoSize - 5);

				//printf("[송신] bytes : %d , msg : %s\n", dwIoSize, pClientInfo->m_szBuf);
				printf("[송신] bytes : %d , msg : %s(Without Header)\n", dwIoSize, msgWithoutHeader);
			}
			//예외 상황
			else
			{
	...
	void CloseSocket(ClientInfo* pClientInfo, bool bIsForce = false)
	{
		...
		//socketClient소켓의 데이터 송수신을 모두 중단 시킨다.
